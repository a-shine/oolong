...
func syncLastUpdatedTasks(w http.ResponseWriter, r *http.Request, db *gorm.DB) {
	var tasks []Task
	...
	// fmt.Println(lastSynced)
	db.Where("updated_at > ?", lastSyncedTime).Find(&tasks)
	json.NewEncoder(w).Encode(tasks)
}
